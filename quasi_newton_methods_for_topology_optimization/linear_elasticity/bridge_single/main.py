...
u = Function(V)
v = Function(V)
F = alpha * inner(sigma(u), eps(v)) * dx - dot(g, v) * ds(3)
...
bcs = [DirichletBC(V.sub(1), Constant(0.0), BDRY(), method="pointwise")]
...
for algorithm in utils.algorithm_list:
    ...
    top.solve(algorithm=algorithm, atol=0.0, rtol=0.0, angle_tol=1.0, max_iter=250)
